...
//     email: data?.data?.email || "N/A",
//     phone: data?.data?.phone || "N/A",
//     profile: data?.data?.image ? `http://192.168.10.98:3000/${data?.data?.image}` : dashProfile,
//   });
...
//         ...values,
//         image: image, // Add the updated image (if any)
//       };
...
//       navigate("/settings/personal-information"); // Navigate to profile or another page after success
//     } catch (err) {
...
const EditMyProfile = () => {
  ...
  // Update phone number when the user types
  useEffect(() => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      // Append image to the FormData if a new image was selected
      if (imageFile) {
      ...
      // Perform the mutation to update the profile
